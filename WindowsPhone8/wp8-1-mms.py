...
def read_nullterm_unistring(f):
        ...
        try:
            ...
            if flag:
                if (
                    ...
                    print(
                        "Unprintable byte value = "
                        + binascii.hexlify(readchar)
                        + " at "
                        + hex(f.tell() - 2).rstrip("L")
                    )
        except:
    ...
    if unprintablechars:
        print(
            "String substitution(s) due to unrecognized/unprintable characters starting at "
            + hex(begin).rstrip("L")
        )
...
parser.add_option(
    "-s", dest="storefile", action="store", type="string", help="store.vol file"
)
parser.add_option(
    "-o",
    dest="outputfilename",
...
rawstore = (
    fbstore.read()
)  # reads whole file into memory so probably don't want to use this with whole image
...
for ahit in attach_hitlist:
    try:
        ...
        print(
            "Bad Attachment Filename string extraction at/around "
            + hex(ahit).rstrip("L")
        )
        exctype, value = sys.exc_info()[:2]
        print("Exception type = ", exctype, ", value = ", value)
    ...
    if (asize == 0x2A2A2A2A) or (asize == 0):
        ...
        print(
            "Attachment ASIZE ERROR! at offset "
            + hex(ahit - 0x23).rstrip("L")
            + " ... skipping this hit"
        )  # no point going further if asize error? eg can't find msgid
        continue
    else:
...
for hit in sms_hitlist:
    ...
    if phonefield == "":
        ...
        try:
            fbstore.seek(hit)
            timestampSMSraw = find_timestamp(
                fbstore, 0x21, 0x19
            )  # timestamp should start 0x21 (33 dec) bytes before hit
        except:
            print(
                "Bad Recipient Timestamp extraction at/around "
                + hex(hit).rstrip("L")
                + " ... Skipping hit ..."
            )
            exctype, value = sys.exc_info()[:2]
        ...
        if timestampSMSraw != 0:
            ...
            try:
                ...
                timestampSMSstr = datetime.datetime.utcfromtimestamp(
                    timestampSMSraw
                ).isoformat()
            except:
                print(
                    "Bad Recipient Timestamp calculation at/around "
                    + hex(hit).rstrip("L")
                    + " ... Skipping hit ..."
                )
                continue  # skip hit if bad read
    ...
    if storevalue == 7:
        ...
        print(
            "Recipient Store value not valid! Skipping hit at "
            + hex(hit).rstrip("L")
            + "\n"
...
print(
    str(len(ipmmms_hitlist))
    + " IPM.MMS Message hits found in "
    + options.storefile
    + "\n"
)
...
for hit in ipmmms_hitlist:
    ...
    try:
        ...
        funistore.seek(
            hit + 0x11
        )  # should be start of Phone1 string for received MMS or x00 x00 if sent/invalid
        phonefield = read_nullterm_unistring(funistore)
    except:
    ...
    if phonefield == "":
        ...
        try:
            ...
            timestamp3str = datetime.datetime.utcfromtimestamp(
                timestamp3raw
            ).isoformat()
        except:
            print(
                "Bad Sent Message Timestamp3 extraction at "
                + hex(timestamp3off).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
            continue  # skip hit if error
        ...
        try:
            fbstore.seek(hit)
            timestamp2raw = find_timestamp(
                fbstore, 0xD6, 0xCE
            )  # timestamp2 is 0xD6 (214 dec) bytes before "IPM.MMS"
            # print("timestamp2raw = " + hex(timestamp2raw))
            if timestamp2raw != 0:
                # returns ISO UTC time string
                timestamp2str = datetime.datetime.utcfromtimestamp(
                    timestamp2raw
                ).isoformat()
            else:
            ...
            print(
                "Bad Sent Message Timestamp2 extraction for hit at "
                + hex(hit).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
        ...
        try:
            fbstore.seek(hit)
            timestamp1raw = find_timestamp(
                fbstore, 0x196, 0x18E
            )  # timestamp1 is 0x196 (406 dec) bytes before "IPM.MMS"
            if timestamp1raw != 0:
                # print("timestamp1raw = " + hex(timestamp1raw))
                # returns ISO UTC time string
                timestamp1str = datetime.datetime.utcfromtimestamp(
                    timestamp1raw
                ).isoformat()
            ...
            print(
                "Bad Sent Message Timestamp1 extraction for hit at "
                + hex(hit).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
        ...
        try:
            ...
            print(
                "Bad Sent Message Size extraction at "
                + hex(flagoffset + 0x8).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
            continue  # skip hit if error
        ...
        try:
            ...
            print(
                "Bad Sent Message Store extraction at "
                + hex(flagoffset - 0xDA).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
            continue  # skip hit if error
        ...
        if storevalue == 7:
            ...
            try:
                ...
                print(
                    "Bad Sent Message Msgid extraction at "
                    + hex(flagoffset - 0xDE).rstrip("L")
                )
                exctype, value = sys.exc_info()[:2]
                print("Exception type = ", exctype, ", value = ", value)
                continue  # skip hit if error
            ...
            print(
                "Sent Message Store value not valid for MMS\n"
        ...
        try:
            ...
            print(
                "Bad Recv Message Phone2/Phone3 extraction around "
                + hex(hit).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
        ...
        try:
            ...
            print(
                "Bad Recv Message Timestamp3 extraction at "
                + hex(hit + 0xE).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
            continue  # skip hit if error
        ...
        #
        time2offsetdelta = (
            (len(phonestr) + 1) * 2 + 1 + 0xEA
        )  # There are 0xEA (234 dec) bytes between start of Timestamp2 and start of Phone0
        timestamp2str = "NA"
        try:
            fbstore.seek(hit)
            timestamp2raw = find_timestamp(
                fbstore, time2offsetdelta, time2offsetdelta - 8
            )  # fd, max offset back, min offset back
            if timestamp2raw != 0:
                # print("timestamp2raw = " + hex(timestamp2raw))
                # returns ISO UTC time string
                timestamp2str = datetime.datetime.utcfromtimestamp(
            ...
            print(
                "Bad Recv Message Timestamp2 extraction for hit around "
                + hex(hit).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
        ...
        try:
            ...
            print(
                "Bad Recv Message Timestamp1 extraction for hit at "
                + hex(time1offset).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
            continue  # skip hit if error
        ...
        try:
            ...
            print(
                "Bad Recv Message Size extraction at "
                + hex(flagoffset + 0x8).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
        ...
        try:
            ...
            print(
                "Bad Recv Message Store extraction at "
                + hex(flagoffset - 0xDA).rstrip("L")
            )
            exctype, value = sys.exc_info()[:2]
            print("Exception type = ", exctype, ", value = ", value)
        ...
        if storevalue == 7:
            ...
            try:
                ...
                print(
                    "Bad Recv Message Msgid extraction at "
                    + hex(flagoffset - 0xDE).rstrip("L")
                )
                exctype, value = sys.exc_info()[:2]
                print("Exception type = ", exctype, ", value = ", value)
            ...
            print(
                "Recv Message Store value not valid for MMS\n"
            )  # could be unexpected layout or attachment for another app (eg email)
            continue
...
for j in sorted_messages_keys:
    ...
    try:
        ...
        for k in range(
            len(attachments[j])
        ):  # there may be more than 1 attachment per msgid/MMS
            # Print order (for each attachment) becomes printmsg, typeval, filesize, file0, file1, file2
            finalstr = (
                printmsgstr
                + str(attachments[j][k][3])
                + "\t"
                + str(attachments[j][k][4])
                + "\t"
                + str(attachments[j][k][0])
                + "\t"
                + str(attachments[j][k][1])
                + "\t"
                + str(attachments[j][k][2])
                + "\n"
            )
            print(finalstr)
            if options.outputfilename != None:
        ...
        print(
            "Problems finding/writing Attachment entries for msgid = "
            + str(j)
            + " ... Skipping\n"
        )
        exctype, value = sys.exc_info()[:2]
