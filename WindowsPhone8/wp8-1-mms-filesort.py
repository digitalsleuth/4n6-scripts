    ...
    for name in files:
        ...
        if name.endswith("73701.dat"):  # MMS specific file naming convention
            ...
            if not (modtime in timestamplist):
                ...
                filelist[modtime] = (
                    list()
                )  # there may be more than one file per modtime so use a list
...
if options.outputHTML != None:
    outputHTML.write(
        '<html><table border="3" style="width:100%"><tr>'
...
for j in sortedkeys:  # each date key
    ...
    for k in range(len(filelist[j])):  # each filename with same date key
        ...
        try:
            ...
            contents = (
                fb.read()
            )  # reading the whole file shouldn't be memory intensive for small files
            sha256hashstr = (
        ...
        if fsize > 11:  # need 12 bytes minimum for "<smil>"
            try:
                ...
                b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11 = struct.unpack(
                    "BBBBBBBBBBBB", contents[0:12]
                )
        ...
        if (
            is_smil == 0
        ):  # Not a <smil>, assume its an attachment (eg text, jpeg/png picture, video, voicenote)
            if fsize > 2:
                try:
                    ...
                    print(
                        "Bad JPG filetype detection for file = "
                        + filelist[j][k]
                        + " with size = "
                        + str(fsize)
                    )
            ...
            if (fsize > 5) and (
                is_jpg == 0
            ):  # Voicenote / AMR file signature is 0x23 0x21 0x41 0x4D 0x52
                try:
                    ...
                    print(
                        "Bad AMR filetype detection for file = "
                        + filelist[j][k]
                        + " with size = "
                        + str(fsize)
                    )
                    ...
                    continue
            if (
                (fsize > 8) and (is_jpg == 0) and (is_amr == 0)
            ):  # PNG file signature is 0x89 0x50 0x4E 0x47 0x0D 0x0A 0x1A 0x0A
                try:
                    b0, b1, b2, b3, b4, b5, b6, b7 = struct.unpack(
                        "BBBBBBBB", contents[0:8]
                    )
                    if (
                        b0 == 0x89
                        and b1 == 0x50
                    ...
                    print(
                        "Bad PNG filetype detection for file = "
                        + filelist[j][k]
                        + " with size = "
                        + str(fsize)
                    )
                    ...
                    continue
            if (
                (fsize > 12) and (is_jpg == 0) and (is_amr == 0) and (is_png == 0)
            ):  # Video MMS can be MP4/QT. The file signature is 0x00 0x00 0x00 0x18 0x66 0x74 0x79 0x70 0x6D 0x70 0x34 0x32.
                try:
                    b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11 = struct.unpack(
                        "BBBBBBBBBBBB", contents[0:12]
                    )
                    ...
                    print(
                        "Bad MP4 filetype detection for file = "
                        + filelist[j][k]
                        + " with size = "
                        + str(fsize)
                    )
            ...
            if (
                ...
                and (is_png == 0)
                and (is_mp4 == 0)
            ):  # VCARD signature is 0x42 0x45 0x47 0x49 0x4E 0x3A 0x56 0x43 0x41 0x52 0x44 0x0D 0x0A.
                try:
                    b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12 = (
                        struct.unpack("BBBBBBBBBBBBB", contents[0:13])
                    )
                    if (
                        b0 == 0x42
                        and b1 == 0x45
                    ...
                    print(
                        "Bad VCARD filetype detection for file = "
                        + filelist[j][k]
                        + " with size = "
                        + str(fsize)
                    )
...
if options.outputHTML != None:
    outputHTML.write(
        '</table><p><b>Note1: "Unknown" Types possibly indicate MMS message text files</b></p>'
