    ...
    while flag:
        try:
            ...
            if readchar not in string.printable:
                ...
                (quotecount % 2) == 0
            ):  # bailout if , char and not in " quotes
                flag = False
            elif flag:
                readstrg += readchar
        except:
...
if options.unicode:
    ...
    author_fbid_label_esc = (
        "\x5C\x22\x61\x75\x74\x68\x6F\x72\x5F\x66\x62\x69\x64\x5C\x22\x3A"
    )
...
if options.unicode:
    ...
    author_name_label_esc = (
        "\x5C\x22\x61\x75\x74\x68\x6F\x72\x5F\x6E\x61\x6D\x65\x5C\x22\x3A"
    )
...
for idx in range(len(author_fbid_hits)):
    if options.unicode:
        ...
        if closest_author_name_hit != 0:
            ...
            print(
                "No corresponding author_name field found in range of hit at "
                + hex(author_fbid_hits[idx]).rstrip("L")
            )
    ...
    if (timestamp_src != "Error!") and (timestamp_src != "NOT_EXTRACTED"):
        try:
            ...
            timestamp_str = datetime.datetime.utcfromtimestamp(
                timestamp_flt
            ).isoformat()
        except:
            exctype, value = sys.exc_info()[:2]
            ...
            timestamp_str = (
                "Error"  # if we get here, the date at this offset is not valid
            )
    else:
...
for idx in range(len(author_fbid_esc_hits)):
    if options.unicode:
        ...
        for i in range(len(author_name_esc_hits)):
            if (author_name_esc_hits[i] > author_fbid_esc_hits[idx]) and (
                (author_name_esc_hits[i] - author_fbid_esc_hits[idx])
                < 2 * AUTHOR_NAME_ESC_FUDGE
            ):
        ...
        if closest_author_name_hit != 0:
            ...
            print(
                "No corresponding author_name field found in range of hit at "
                + hex(author_fbid_esc_hits[idx]).rstrip("L")
            )
        ...
        for i in range(len(timestamp_esc_hits)):
            if (timestamp_esc_hits[i] > author_fbid_esc_hits[idx]) and (
                (timestamp_esc_hits[i] - author_fbid_esc_hits[idx])
                < 2 * TIMESTAMP_ESC_FUDGE
            ):
    ...
    messages[author_fbid_esc_hits[idx]] = (
        author_fbid,
        author_name,
        message,
        timestamp_src,
