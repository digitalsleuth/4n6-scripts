...
def rev_extract_unistring(f):
    ...
    while flag:
        try:
            readchar = f.read(1)
            charcount += 1
            if (readchar == unichr(0)) and (
                charcount > 1
            ):  # bailout if null char and not null at end of string
                flag = False
            else:
                if (readchar == unichr(0)) and (
                    charcount == 1
                ):  # skip null at end of string
...
for hit in hits:
    ...
    idoffset_end = (
        funi.tell() - 0x15
    )  # ass-umes end of ID string is fixed offset away from first byte of Phone1
    ...
    afterstop_offset = (
        fb.tell()
    )  # cursor should be at byte just after the stop timeoffset
    ...
    if stoptimeval != 0:
        ...
        # Read start time
        startoffset = (
            afterstop_offset - 0x10
        )  # start of Start timestamp is 0x10 bytes before end of Stop timestamp
        # print("startoffset = " + hex(startoffset))
        fb.seek(startoffset)
        starttimeval = read_filetime(fb)
        if starttimeval != 0:
            try:
                starttimestring = datetime.datetime.utcfromtimestamp(
                    starttimeval
                ).isoformat()
                valid_startime = True
            except:
...
sorted_calls_keys = sorted(
    call_entries, key=lambda x: (call_entries[x][1], call_entries[x][1])
)
