...
sorted_contact_keys = sorted(
    contact_entries, key=lambda x: (contact_entries[x][1], contact_entries[x][1])
)
...
if options.tsvfile != None:
    ...
    tsvof.write(
        "Offset\tLast_Field(Name)\tSecond_Last_Field(Name)\tThird_Last_Field(Phone)\n"
    )
    ...
    for key in sorted_contact_keys:
        ...
        tsvof.write(
            hex(key).rstrip("L")
            + "\t"
            + contact_entries[key][0]
            + "\t"
            + contact_entries[key][1]
            + "\t"
            + contact_entries[key][2]
            + "\n"
        )
