...
def read_nullterm_unistring(f):
    ...
    if unprintablechars:
        print(
            "String substitution(s) due to unrecognized/unprintable characters at "
            + hex(begin).rstrip("L")
        )
...
def find_flag(f):
    ...
    fb.seek(
        begin - 0x11
    )  # usually the 3rd last digit offset but can be the last digit eg 1SEP DUB data
...
def sliceNsearchRE(fd, chunksize, delta, term):
    ...
    pattern = re.compile(
        term, re.DOTALL
    )  # should only really call this once at start, if same substring.
    ...
    if chunksize >= stats.st_size:
        ...
        for chunknum in range(numchunks):
            if (chunk_size_to_read + begin_chunk) > stats.st_size:
                chunk_size_to_read = (
                    stats.st_size - begin_chunk
                )  # last chunk only gets until end of file
...
substring1 = (
    "\x53\x00\x4d\x00\x53\x00\x74\x00\x65\x00\x78\x00\x74\x00\x00\x00"  # ie "SMStext"
)
...
for smshit in smshits:
    ...
    if val == 0x40:
        ...
        if smstimeval != 0:
            try:
                # returns UTC time
                smstimestring = datetime.datetime.utcfromtimestamp(
                    smstimeval
                ).isoformat()
            except:
...
for hit in hits:
    ...
    if nums_listed == 0:
        ...
        timeval = find_timestamp(
            fb, 0xEA + 0x5, 0x7D
        )  # Based on 30AUG DUB data, change the max offset to 0xEA + 5, Based on 1SEP data change min to x7D (from 0xAF)
    if nums_listed == 1:
        ...
        timeval = find_timestamp(
            fb, 0x120, 0x9B
        )  # Based on Garda test data changed max offset to 0x120
...
sorted_messages_keys = sorted(
    sms_entries, key=lambda x: (sms_entries[x][0], sms_entries[x][0])
)
