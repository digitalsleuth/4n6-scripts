...
def find_type_size(strg):
    ...
    if strg.upper() == ">Q" or strg.upper() == "Q" or strg.upper() == "<Q":
        ...
        "OSX32" in strg.upper()
        or "UNIX32" in strg.upper()
        or "GPS32" in strg.upper()
        or "AOL32" in strg.upper()
        or "HFS" in strg.upper()
    ):
        if strg.startswith(">"):
            return ">I", 4
        ...
        return (
            "6B",
            6,
        )  # this is just a placeholder so UNIX48MS doesn't return 0 (error)
...
def calc_date(val, field):
    ...
    if "OSX32" in type_dict[field].upper():
        ...
        try:
            datetimestr = datetime.datetime.fromtimestamp(val + 978307200).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        return datetimestr
    elif ("UNIX32" in type_dict[field].upper()) or (
        "UNIX10DIGDEC" in type_dict[field].upper()
    ):
        # print "UNIX date string is: " + datetime.datetime.fromtimestamp(val).strftime('%Y-%m-%dT%H:%M:%S')
        try:
            datetimestr = datetime.datetime.fromtimestamp(val).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        ...
        try:
            datetimestr = datetime.datetime.fromtimestamp(val + 315964800).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        ...
        try:
            datetimestr = datetime.datetime.fromtimestamp(val + 315532800).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        ...
        try:
            datetimestr = datetime.datetime.fromtimestamp(val - 2082844800).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        return datetimestr
    elif ("UNIX48MS" in type_dict[field].upper()) or (
        "UNIX13DIGDEC" in type_dict[field].upper()
    ):
        # print "UNIX48MS/UNIX13DIGDEC date string is: " + datetime.datetime.fromtimestamp(val/1000.0).strftime('%Y-%m-%dT%H:%M:%S')
        try:
            datetimestr = datetime.datetime.fromtimestamp(val / 1000.0).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        ...
        try:
            ...
            datetimestr = datetime.datetime(year, mm, dd, hh, mn, ss).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        ...
        try:
            ...
            datetimestr = datetime.datetime(yy, mm, dd, hh, mn, ss).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        except:
            datetimestr = "Unknown"
        ...
        ss = (timehalf & 0b11111) * 2
        # LSB 5bits (0...4) x 2 equals secs
        if ss == 60:
            ss = 59
        mn = (timehalf & 0b11111100000) >> 5
        # 6bits (5...10) equals minutes
        hh = (timehalf & 0b1111100000000000) >> 11
        # 5bits (11...15) equals hours
        dd = datehalf & 0b11111
        # 5bits (0...4) equals day
        mm = (datehalf & 0b111100000) >> 5
        # 4bits (5...8) equals month
        yy = ((datehalf & 0b1111111000000000) >> 9) + 1980
        # 7bits (9...16)+1980 equals year
...
def extract_unix10digdec(f):
    ...
    try:
        unix_decimal_10_int = int(
            unix_decimal_10_string
        )  # now convert string "1170245478" to decimal int
    except:
...
def extract_unix13digdec(f):
    ...
    try:
        unix_decimal_13_int = int(
            unix_decimal_13_string
        )  # now convert string "01170245478000" to decimal int
    except:
...
def extract_defined_string(field, f, filename):
    ...
    if type_dict[field].upper() == "S":
        pattern = (
            num_types_dict[field] + type_dict[field]
        )  # eg pattern is "140s" for "140 | s" template
        try:
            ...
            if all(c in string.printable for c in value):
                ...
                value = ""
                print(
                    filename + ":" + str(fieldoffset) + " " + field + " is unprintable"
                )
        except:
            value = ""
            print(
    ...
    elif (type_dict[field].upper() == "UTF16BE") or (
...
def extract_deferred_string(field, f, size, filename):
    ...
    if type_dict[field].upper() == "S":
        pattern = (
            str(size) + "s"
        )  # eg pattern is "140s" for "msgsize | s" template where msgsize = 140
        try:
            value = struct.unpack(pattern, f.read(size))[0]
            # ensure string is printable
            if all(c in string.printable for c in value):
            ...
            print(
                ...
                + " "
                + field
                + " - Error extracting deferred string"
            )
    elif (type_dict[field].upper() == "UTF16BE") or (
        type_dict[field].upper() == "UTF16LE"
    ):
...
def parse_record(f, hit):
    extracted_vals = (
        {}
    )  # local dict of numerical extracted values keyed by field name (used for storing/retrieving deferred sized strings)
    ...
    for field in field_names:
        ...
        if type_dict[field].upper() == "X":  # Dont care about these X bytes ...
            ...
            if not num_types_dict[field].isdigit():
                ...
                skipsize = int(
                    extracted_vals[num_types_dict[field]]
                )  # retrieve size from previously extracted values dict
            else:
            ...
            if (fieldoffset + skipsize) < fileinfo.st_size:
                ...
                print(
                    ...
                    + field
                    + " - specified offset ("
                    + str(fieldoffset + skipsize)
                    + ") too large!"
                )
                return False  # Bailout of function cos something is wrong
        elif (num_types_dict[field] == "0") and (
            (type_dict[field].upper() == "S")
            or (type_dict[field].upper() == "UTF16BE")
            or (type_dict[field].upper() == "UTF16LE")
        ):
        ...
        elif (not num_types_dict[field].isdigit()) and (
            (type_dict[field].upper() == "S")
            or (type_dict[field].upper() == "UTF16BE")
            or (type_dict[field].upper() == "UTF16LE")
        ):
            ...
            if num_types_dict[field] in extracted_vals:
                ...
                print(
                    num_types_dict[field]
                    + " is unknown and cannot be used to extract the "
                    + field
                    + " field ... skipping"
                )
        elif (num_types_dict[field].isdigit()) and (
            (type_dict[field].upper() == "S")
            or (type_dict[field].upper() == "UTF16BE")
            or (type_dict[field].upper() == "UTF16LE")
        ):
            ...
            try:
                ...
                extracted_vals[field] = (
                    value  # store numerical values in case we need it later eg deferred string sizes
                )
                # print "extracted field = " + field + "... value = " + str(value)
            except:
                print(
                    "Error extracting data! Offset = "
                    + str(fieldoffset)
                    + ", Field = "
                    + field
                )
            ...
            if (
                "OSX32" in type_dict[field].upper()
                or "UNIX32" in type_dict[field].upper()
                or "GPS32" in type_dict[field].upper()
                or "AOL32" in type_dict[field].upper()
                or "HFS32" in type_dict[field].upper()
            ):
                datefield = calc_date(value, field)
                print(
                    filename
...
parser.add_option(
    "-d", dest="defn", action="store", type="string", help="Template Definition File"
)
parser.add_option(
    "-f",
    dest="filename",
