...
parser.add_option(
    "-t", dest="threadsdb", action="store", type="string", help="threads_db2 input file"
)
parser.add_option(
    "-c",
    dest="contactsdb",
...
while row:
    ...
    try:
        ...
        print(
            f"Could not extract contact data for contact_id = {row[CONTACTS_QUERY_CONTACTS_ID_COL_IDX]}"
        )
        row = contactscursor.fetchone()
    ...
    coverphotouri = "NA"
    if (
        decoded_data["timelineCoverPhoto"] is not None
    ):  # ie if timelineCoverPhoto not null
        try:
            ...
            print(
                f"No timelineCoverPhoto available for contact_id {row[CONTACTS_QUERY_CONTACTS_ID_COL_IDX]}"
            )
    ...
    contactsdict[row[0]] = (
        decoded_data["profileFbid"],
        decoded_data["name"]["displayName"],
        displaynum,
        univnum,
        decoded_data["smallPictureUrl"],
        decoded_data["bigPictureUrl"],
        decoded_data["hugePictureUrl"],
        coverphotouri,
    )
...
sorted_contact_keys = sorted(
    contactsdict, key=lambda x: contactsdict[x][CONTACTS_DISPLAY_NAME_ID_IDX]
)
...
for key in sorted_contact_keys:
    print(
        key
        + "\t"
        + contactsdict[key][CONTACTS_PROFILE_ID_IDX]
        + "\t"
        + contactsdict[key][CONTACTS_DISPLAY_NAME_ID_IDX]
        + "\t"
        + contactsdict[key][CONTACTS_DISPLAY_PHONE_IDX]
        + "\t"
        + contactsdict[key][CONTACTS_UNIV_PHONE_IDX]
        + "\t"
        + contactsdict[key][CONTACTS_SML_PIC_URL_IDX]
        + "\t"
        + contactsdict[key][CONTACTS_BIG_PIC_URL_IDX]
        + "\t"
        + contactsdict[key][CONTACTS_HUGE_PIC_URL_IDX]
        + "\t"
        + contactsdict[key][CONTACTS_COVER_PIC_IDX]
        + "\n"
    )
...
while row:
    ...
    messagesdict[row[MESGS_QUERY_MSG_ID_COL_IDX]] = (
        row[MESGS_QUERY_THREAD_ID_COL_IDX],
        textstr,
        sender,
        parties,
        row[MESGS_QUERY_TIMESTAMP_COL_IDX],
        sourcestr,
        coords_lat,
        coords_long,
        coords_accuracy,
...
sorted_messages_keys = sorted(
    messagesdict,
    key=lambda x: (
        messagesdict[x][MESGS_THR_ID_IDX],
        messagesdict[x][MESGS_TIMESTAMP_IDX],
    ),
)
...
for key in sorted_messages_keys:
    ...
    if messagesdict[key][MESGS_TIMESTAMP_IDX] > 0:
        datetimestr = datetime.datetime.fromtimestamp(
            messagesdict[key][MESGS_TIMESTAMP_IDX] / 1000
        ).strftime("%Y-%m-%dT%H:%M:%S")
    else:
        datetimestr = str(
            messagesdict[key][MESGS_TIMESTAMP_IDX]
        )  # if 0, just print raw value.
    ...
    if (messagesdict[key][MESGS_LAT_IDX] != "NA") and (
        ...
        latlongurl = (
            ...
            + str(messagesdict[key][MESGS_LAT_IDX])
            + ",+"
            + str(messagesdict[key][MESGS_LONG_IDX])
            + "+%28"
            + urllib.quote_plus(
                str(messagesdict[key][MESGS_TEXT_IDX] + " @" + datetimestr)
            )
            + "%29&iwloc=A&hl=en"
        )
    ...
    print(
        key
        + "\t"
        + messagesdict[key][MESGS_THR_ID_IDX]
        + "\t"
        + messagesdict[key][MESGS_TEXT_IDX]
        + "\t"
        + messagesdict[key][MESGS_SENDER_IDX]
        + "\t"
        + partieslist
        + "\t"
        + datetimestr
        + "\t"
        + messagesdict[key][MESGS_SOURCE_IDX]
        + "\t"
        + str(messagesdict[key][MESGS_LAT_IDX])
        + "\t"
        + str(messagesdict[key][MESGS_LONG_IDX])
        + "\t"
        + str(messagesdict[key][MESGS_ACCURACY_IDX])
        + "\t"
        + str(messagesdict[key][MESGS_HEADING_IDX])
        + "\t"
        + str(messagesdict[key][MESGS_SPEED_IDX])
        + "\t"
        + str(messagesdict[key][MESGS_ALTITUDE_IDX])
        + "\t"
        + latlongurl
        + "\n"
    )
