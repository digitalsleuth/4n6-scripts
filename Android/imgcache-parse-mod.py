...
parser.add_option(
    "-o", dest="htmlfile", action="store", type="string", help="HTML table File"
)
parser.add_option(
    "-p",
    dest="parsepicsonly",
...
if (options.parsepicsonly) and (options.parsevidsonly):
    ...
    print(
        "When -p and -v are both not specified, script parses for both picture and video cache items"
    )
    exit(-1)
...
outputdict = (
    {}
)  # dictionary sorted by JPG offset. Contains extracted filename, size, item path string and MD5 tuple.
...
for hit in hits:
    ...
    recsize = struct.unpack("<I", fb.read(4))[
        0
    ]  # size does NOT include these 4 bytes. From start of path string to xFFD9 at end of JPG file
    jpgend = hit + recsize + 1  # should point to the byte after FFD9
    if jpgend > filesize + 1:
        print(
            "Bad end of JPG offset calculated for JPG starting at "
            + hex(hit).rstrip("L").upper()
            + " ... skipping!\n"
        )
        break
    ...
    try:
        timestring = datetime.datetime.utcfromtimestamp(timestamp / 1000).strftime(
            "%Y-%m-%dT%H-%M-%S"
        )
    except:
        timestring = "Error"
    ...
    if jpgstart > 0:
        ...
        print(
            "JPG output size(bytes) = "
            + str(len(rawjpgoutput))
            + " from offset = "
            + hex(jpgstart).rstrip("L").upper()
            + "\n"
        )
        ...
        with open(outputfilename, "rb") as pic:
            ...
            md5.update(
                pic.read()
            )  # file shouldn't be that large so just read into memory
